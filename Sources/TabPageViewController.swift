...
open class TabPageViewController: UIPageViewController {
    ...
    open var tabItems: [(viewController: UIViewController, title: String,subTitle : String )] = []
 
    open var tabDelegate : TabPageDelegate?
...
public extension TabPageViewController {
    ...
    func displayControllerWithIndex(_ index: Int, direction: UIPageViewController.NavigationDirection, animated: Bool) {
        ...
        else if option.enabledIndexes != nil && index < option.enabledIndexes! && index >= option.minIndex{
            beforeIndex = index
            shouldScrollCurrentBar = false
            let nextViewControllers: [UIViewController] = [tabItems[index].viewController]
...
extension TabPageViewController {
    ...
    fileprivate func updateNavigationBar() {
        if let navigationBar = navigationController?.navigationBar {
            ...
            navigationBar.setBackgroundImage(option.navigationBarImage, for: .default)
    ...
    fileprivate func configuredTabView() -> TabView {
        ...
        tabView.pageTabSubItems = tabItems.map({ $0.subTitle})
        ...
        tabView.pageItemPressedBlock = { [weak self] (index: Int, direction: UIPageViewController.NavigationDirection) in
            ...
            self?.tabDelegate?.didMoveToPage(controller: (self?.tabItems[index].viewController)!, index: index)
        }
...
extension TabPageViewController: UIPageViewControllerDataSource {
    ...
    fileprivate func nextViewController(_ viewController: UIViewController, isAfter: Bool) -> UIViewController? {
        ...
        if option.enabledIndexes != nil{
            if index >= option.minIndex && index < option.enabledIndexes! {
...
extension TabPageViewController: UIScrollViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        tabView.scrollCurrentBarView(index, contentOffsetX: scrollOffsetX < 0 ? 0 : scrollOffsetX)
    }

    public func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        tabView.updateCurrentIndex(currentIndex ?? 0, shouldScroll: true)
    }
}
