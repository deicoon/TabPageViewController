...
internal class TabView: UIView {
    ...
    public func updateMinIndexs(index : Int){
...
extension TabView {
    ...
    func scrollCurrentBarView(_ index: Int, contentOffsetX: CGFloat) {
        ...
        if let currentCell = collectionView.cellForItem(at: currentIndexPath) as? TabCollectionCell, let nextCell = collectionView.cellForItem(at: nextIndexPath) as? TabCollectionCell {
            ...
            let width = (abs(scrollRate) * (nextCell.frame.width - currentCell.frame.width) ) - 2 * option.currentBarPadding
            if isInfinity {
                let scroll = scrollRate * distance
                collectionView.contentOffset.x = collectionViewContentOffsetX + scroll
            } else {
                if scrollRate > 0 {
                    currentBarViewLeftConstraint?.constant = currentCell.frame.minX + scrollRate * currentCell.frame.width + option.currentBarPadding
                } else {
                    currentBarViewLeftConstraint?.constant = currentCell.frame.minX + nextCell.frame.width * scrollRate + option.currentBarPadding
                }
            }
    ...
    func updateCurrentIndex(_ index: Int, shouldScroll: Bool) {
        if  option.enabledIndexes != nil , index < option.enabledIndexes! , index >= option.minIndex{
    ...
    fileprivate func moveCurrentBarView(_ indexPath: IndexPath, animated: Bool, shouldScroll: Bool) {
        if let enabledIndex = option.enabledIndexes, indexPath.item < enabledIndex , indexPath.item >= option.minIndex{
            if shouldScroll {
            ...
            if let cell = collectionView.cellForItem(at: indexPath) as? TabCollectionCell {
                ...
                currentBarViewWidthConstraint.constant = cell.frame.width - 2 * option.currentBarPadding
                if !isInfinity {
                    currentBarViewLeftConstraint?.constant = cell.frame.origin.x + option.currentBarPadding
                }
...
extension TabView: UICollectionViewDataSource {
    ...
    fileprivate func configureCell(_ cell: TabCollectionCell, indexPath: IndexPath) {
        ...
        //        cell.showSubItem = option.showTabSubItem
        cell.subItem = pageTabSubItems[fixedIndex]
